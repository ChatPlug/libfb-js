...
class TCompactProtocol {
  ...
  lastField_: any[];
  lastFieldId_: number;
  string_limit_: number;
  string_buf_: any;
  string_buf_size_: number;
  container_limit_: number;
  booleanField_: { name: any; hasBoolValue: boolean };
  boolValue_: { hasBoolValue: boolean; boolValue: boolean };
  static TTypeToCType: any;
  static Types: any;
  static VERSION_N: any;
  static VERSION_MASK: any;
  static TYPE_SHIFT_AMOUNT: number;
  static TYPE_MASK: number;
  _seqid: any;
  static TYPE_BITS: number;
  constructor(trans: object) {
    this.trans = trans;
    this.lastField_ = [];
    this.lastFieldId_ = 0;
   ...
   */
  getCompactType(ttype: number): number {
  ...
  static PROTOCOL_ID(PROTOCOL_ID: any): any {
   ...
   */
  writeFieldBegin(name: string, type: number, id: number) {
    if (type != Type.BOOL) {
   ...
   */
  writeMapBegin(keyType: number, valType: number, size: number) {
    if (size === 0) {
      this.writeByte(0);
    } else {
   ...
   */
  writeSetBegin(elemType: number, size: number) {
    this.writeCollectionBegin(elemType, size);
  }
  writeSetEnd() {}
  writeBool(value) {
  ...
  writeStringOrBinary(name, encoding, arg) {
